...
function styleFromPreset(preset) {
  switch (preset) {
    case 'luxury': return { bg: '#f7f1e4', accent: '#7a3f1e', font: 'Scheherazade New' };
    case 'dark': return { bg: '#efe7da', accent: '#53311f', font: 'Tajawal' };
    case 'minimal': return { bg: '#f8f8f6', accent: '#2f3d4d', font: 'Cairo' };
    case 'classic': return { bg: '#f6f1e8', accent: '#6a3b1f', font: 'Noto Naskh Arabic' };
    default:
      return {
...
function ensureFontOptions() {
  ...
  const existing = new Set(Array.from(els.fontSelect.options).map(o => o.value));
  EXTRA_FONTS.forEach(font => {
    if (existing.has(font.value)) return;
...
<link href="https://fonts.googleapis.com/css2?family=Amiri:wght@400;700&family=Aref+Ruqaa:wght@400;700&family=Cairo:wght@400;700&family=Harmattan:wght@400;500;700&family=Markazi+Text:wght@400;500;600;700&family=Marhey:wght@400;700&family=Noto+Kufi+Arabic:wght@400;500;700&family=Noto+Naskh+Arabic:wght@400;500;700&family=Reem+Kufi:wght@400;500;700&family=Scheherazade+New:wght@400;500;700&family=Tajawal:wght@400;700&display=swap" rel="stylesheet">
...
function getSafeTitle() {
  return (els.bookTitle.value.trim() || 'book').replace(/[\\/:*?"<>|]+/g, '-');
}

function getBusyLabel(kind) {
  if (kind === 'zip') return 'تنزيل كل صفحات المعاينة ZIP';
  if (kind === 'page') return 'تنزيل الصفحة';
  return 'إنشاء الكتاب وتنزيله';
}

function getAdaptiveScale(pageCount) {
  if (pageCount >= 300) return EXPORT_SCALE_LARGE;
  if (pageCount >= 120) return EXPORT_SCALE_MEDIUM;
  return EXPORT_SCALE;
}
...
function updateBusyProgress(kind, current, total) {
  if (kind === 'pdf') {
    els.exportBtn.textContent = `جارٍ إنشاء PDF (${current}/${total})`;
    return;
  }
  if (kind === 'zip') {
    els.downloadZipBtn.textContent = `جارٍ تجهيز ZIP (${current}/${total})`;
  }
}

function setBusyState(busy, kind = 'pdf') {
  state.isExporting = busy;
  els.exportBtn.disabled = busy;
  els.downloadZipBtn.disabled = busy;
  els.refreshPreview.disabled = busy;
  if (!busy) {
    els.exportBtn.textContent = getBusyLabel('pdf');
    els.downloadZipBtn.textContent = getBusyLabel('zip');
  } else {
    if (kind === 'zip') {
      els.downloadZipBtn.textContent = 'جارٍ تجهيز ZIP...';
    } else {
      els.exportBtn.textContent = 'جارٍ إنشاء PDF...';
    }
  }
}
...
async function renderPageToCanvas(pageElement, scale = EXPORT_SCALE) {
  return await window.html2canvas(pageElement, {
    scale,
    useCORS: true,
    allowTaint: true,
...
async function renderPageToDataUrl(pageElement, format = 'image/png', quality = 1) {
...
async function exportPdf(safeTitle) {
  if (!window.jspdf?.jsPDF) {
    throw new Error('jsPDF library is missing');
  }

  const root = await createLockedExportRoot();
  try {
    const pages = Array.from(root.querySelectorAll('.book-page'));
    const scale = getAdaptiveScale(pages.length);
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'a5', compress: true });
    const pageWidth = pdf.internal.pageSize.getWidth();
    const pageHeight = pdf.internal.pageSize.getHeight();

    for (let index = 0; index < pages.length; index += 1) {
      updateBusyProgress('pdf', index + 1, pages.length);
...
async function downloadPreviewPage(pageNo, safeTitle) {
  ...
  try {
    ...
    const canvas = await renderPageToCanvas(pageElement, EXPORT_SCALE);
    const blob = await canvasToBlob(canvas, 'image/png');
    canvas.width = 1;
    canvas.height = 1;
    downloadFile(blob, `${safeTitle}-page-${String(pageNo).padStart(3, '0')}.png`);
  } finally {
    root.remove();
  }
}
...
async function downloadPagesZip(safeTitle) {
  ...
  try {
    ...
    const useJpeg = pages.length > 120;
    ...
    for (let index = 0; index < pages.length; index += 1) {
      updateBusyProgress('zip', index + 1, pages.length);
      const canvas = await renderPageToCanvas(pages[index], scale);
      const blob = await canvasToBlob(canvas, mime, quality);
      canvas.width = 1;
      canvas.height = 1;
      zip.file(`${safeTitle}-page-${String(index + 1).padStart(4, '0')}.${ext}`, blob, { binary: true });
      await yieldToUI();
    }
