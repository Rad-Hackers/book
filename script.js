...
const els = {
  ...
  previewCoverArt: document.getElementById('previewCoverArt'),
...
function buildPageHtml(pageNo) {
  return `<div class="page-number">${pageNo}</div>`;
}
...
function styleFromPreset(preset) {
  switch (preset) {
    case 'luxury':
      return { bg: '#f7f1e4', accent: '#7a3f1e', font: 'Amiri' };
    case 'dark':
      return { bg: '#efe7da', accent: '#53311f', font: 'Tajawal' };
    case 'minimal':
      return { bg: '#f8f8f6', accent: '#2f3d4d', font: 'Cairo' };
    case 'classic':
      return { bg: '#f6f1e8', accent: '#6a3b1f', font: 'Amiri' };
    default:
      return {
...
function applyBookTheme() {
  const preset = els.stylePreset.value;
  const presetVals = styleFromPreset(preset);
...
function syncStats() {
  ...
  const totalWords = state.parts.reduce((sum, part) => sum + countWords(part.content), 0);
  els.wordsCount.textContent = totalWords.toLocaleString('en-US');
}
...
function splitPartIntoPages(title, content) {
  ...
  if (!paragraphs.length) {
    return [{ title, paragraphs: ['لا يوجد نص في هذا البارت.'], isContinuation: false }];
  }
  ...
  for (const paragraph of paragraphs) {
    ...
    const limit = firstPage ? 190 : 250;
    if (current.length && currentWords + words > limit) {
      flush(!firstPage);
      firstPage = false;
    }
...
function renderPreview() {
  applyBookTheme();
  els.previewTitle.textContent = els.bookTitle.value.trim() || 'عنوان كتابك';
  els.previewAuthor.textContent = els.bookAuthor.value.trim() || 'اسم المؤلف';
  els.previewDescription.textContent = els.bookDescription.value.trim() || 'اكتب وصف الكتاب من الشريط الجانبي لتظهر المعاينة هنا بشكل أنيق.';

  if (state.coverDataUrl) {
    els.previewCoverArt.style.backgroundImage = `linear-gradient(180deg, rgba(0,0,0,.08), rgba(0,0,0,.55)), url('${state.coverDataUrl}')`;
  } else {
    els.previewCoverArt.style.backgroundImage = '';
  }
  ...
  let pageNo = 4;
  state.parts.forEach((part, index) => {
    const pages = splitPartIntoPages(part.title || `البارت ${index + 1}`, part.content || '');

    const tocItem = document.createElement('div');
    tocItem.className = 'toc-item';
    tocItem.innerHTML = `<span class="chapter-num">${index + 1}</span><span>${escapeHtml(part.title || `البارت ${index + 1}`)}</span><span>${pageNo}</span>`;
    els.tocList.appendChild(tocItem);

    pages.forEach((pageChunk, pageIndex) => {
      const page = document.createElement('div');
      page.className = 'book-page chapter-page';
      const heading = pageIndex === 0
        ? escapeHtml(pageChunk.title)
        : `${escapeHtml(pageChunk.title)} <span class="continued-mark">— متابعة</span>`;

      const paragraphs = pageChunk.paragraphs
        .map(p => `<p class="${classifyParagraph(p)}">${escapeHtml(p)}</p>`)
        .join('');

      page.innerHTML = `
        <h2>${heading}</h2>
        <div class="chapter-body">${paragraphs}</div>
        ${buildPageHtml(pageNo)}
      `;

      els.chaptersPreview.appendChild(page);
      pageNo += 1;
    });
  });
...
function makeExportHtmlDocument() {
  const title = escapeHtml(els.bookTitle.value.trim() || 'Book');
  renderPreview();
  return `<!doctype html>
...
<div class="export-wrap">${els.previewBook.outerHTML}</div>
</body>
</html>`;
}
...
function makePlainTextExport() {
  const lines = [];
  lines.push(els.bookTitle.value.trim() || 'عنوان الكتاب');
  lines.push(els.bookAuthor.value.trim() || 'اسم المؤلف');
  lines.push('');
  lines.push('الوصف');
  lines.push('------');
  lines.push(els.bookDescription.value.trim() || '');
  lines.push('');
...
function createPrintableBookElement() {
  renderPreview();
  const wrapper = document.createElement('div');
  wrapper.className = 'export-root';
  const clone = els.previewBook.cloneNode(true);
...
async function exportPdf(safeTitle) {
  const wrapper = createPrintableBookElement();
  try {
...
async function exportBook() {
  ...
  if (format === 'txt') {
    const blob = new Blob([makePlainTextExport()], { type: 'text/plain;charset=utf-8' });
    downloadFile(blob, `${safeTitle}.txt`);
    return;
  }

  if (format === 'html') {
    const blob = new Blob([makeExportHtmlDocument()], { type: 'text/html;charset=utf-8' });
    downloadFile(blob, `${safeTitle}.html`);
    return;
  }
...
els.coverInput.addEventListener('change', e => handleCoverUpload(e.target.files?.[0]));
...
els.exportBtn.addEventListener('click', () => exportBook().catch(err => {
  console.error(err);
  alert('حدث خطأ أثناء إنشاء الكتاب. جرّب مرة أخرى. وإذا استمر الخطأ صدّر HTML أولًا ثم افتحه واطبع منه PDF.');
}));
